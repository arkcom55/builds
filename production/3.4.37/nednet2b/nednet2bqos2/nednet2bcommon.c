...
int InitQosConfig(QosConfig_t* qp) {
    if (qp) {
        qp->overBudget = FALSE;
        qp->initialTargetRate = DEFAULT_INITIAL_RATE;
        qp->targetRate = DEFAULT_TARGET_RATE;
        qp->targetRateMultiplier = DEFAULT_TARGET_RATE_MULTIPLIER;
        qp->targetRateDecayTime = DEFAULT_TARGET_RATE_DECAY_TIME;
        qp->drainRateMultiplier = DEFAULT_DRAIN_RATE_MULTIPLIER;
        qp->bucketSize = DEFAULT_BUCKET_SIZE;
        qp->restartSize = DEFAULT_RESTART_SIZE;
    }
    return ERR_OK;
}

//*******************************************************************
// InitQosConfigOverbudget
//-------------------------------------------------------------------
int InitQosConfigOverbudget(QosConfig_t* qp) {
    if (qp) {
        qp->overBudget = FALSE;
        qp->initialTargetRate = DEFAULT_OVERBUDGET_INITIAL_RATE;
        qp->targetRate = DEFAULT_OVERBUDGET_TARGET_RATE;
        qp->targetRateMultiplier = DEFAULT_OVERBUDGET_TARGET_RATE_MULTIPLIER;
        qp->targetRateDecayTime = DEFAULT_OVERBUDGET_TARGET_RATE_DECAY_TIME;
        qp->drainRateMultiplier = DEFAULT_OVERBUDGET_DRAIN_RATE_MULTIPLIER;
        qp->bucketSize = DEFAULT_OVERBUDGET_BUCKET_SIZE;
        qp->restartSize = DEFAULT_OVERBUDGET_RESTART_SIZE;
    }
    return ERR_OK;
}
...
int ReadQosConfig(String file, QosConfig_t* qp) {
    String config = ReadTextFile(file);
    if (config) {            
        cJSON* json = cJSON_Parse(config);
        if (json) {
            cJSON* item;
            
            item = cJSON_GetObjectItemCaseSensitive(json, "overBudget");
            if (item && cJSON_IsNumber(item))
            {
                qp->overBudget = (int)(item->valuedouble);
            }
            
            item = cJSON_GetObjectItemCaseSensitive(json, "initialTargetRateBitsPerSec");
            if (item && cJSON_IsNumber(item))
            {
                qp->initialTargetRate = (long)(item->valuedouble);
            }
            
            item = cJSON_GetObjectItemCaseSensitive(json, "targetRateBitsPerSec");
            if (item && cJSON_IsNumber(item))
            {
                qp->targetRate = (long)(item->valuedouble);
            }
            
            item = cJSON_GetObjectItemCaseSensitive(json, "bucketSizeBytes");
            if (item && cJSON_IsNumber(item))
            {
                qp->bucketSize = (long)(item->valuedouble);
            }
            
            item = cJSON_GetObjectItemCaseSensitive(json, "restartSizeBytes");
            if (item && cJSON_IsNumber(item))
            {
                qp->restartSize = (long)(item->valuedouble);
            }
            
            item = cJSON_GetObjectItemCaseSensitive(json, "targetRateMultiplier");
            if (item && cJSON_IsNumber(item))
            {
                qp->targetRateMultiplier = (float)(item->valuedouble);
            }
            
            item = cJSON_GetObjectItemCaseSensitive(json, "drainRateMultiplier");
            if (item && cJSON_IsNumber(item))
            {
                qp->drainRateMultiplier = (float)(item->valuedouble);
            }
            
            item = cJSON_GetObjectItemCaseSensitive(json, "targetRateDecayTimeSec");
            if (item && cJSON_IsNumber(item))
            {
                qp->targetRateDecayTime = (int)(item->valuedouble);
            }
...
int WriteQosConfig(String file, QosConfig_t* qp) {
    ...
    if (fp) {
        fprintf(fp, "{\n");
        fprintf(fp, "    \"overBudget\":%d,\n", qp->overBudget);
        fprintf(fp, "    \"initialTargetRateBitsPerSec\":%ld,\n", qp->initialTargetRate);
        fprintf(fp, "    \"targetRateBitsPerSec\":%ld,\n", qp->targetRate);
        fprintf(fp, "    \"targetRateMultiplier\":%.1f,\n", qp->targetRateMultiplier);
        fprintf(fp, "    \"targetRateDecayTimeSec\":%d,\n", qp->targetRateDecayTime);
        fprintf(fp, "    \"drainRateMultiplier\":%.1f,\n", qp->drainRateMultiplier);
        fprintf(fp, "    \"bucketSizeBytes\":%ld,\n", qp->bucketSize);
        fprintf(fp, "    \"restartSizeBytes\":%ld\n", qp->restartSize);
        fprintf(fp, "}");
        fclose(fp);
    } else {
        Logger(LOG_ERROR, "nednet2bqos2: Could not create qos config file '%s'.", file);
    }
    return ERR_OK;
}
...
String ConfigRead() {
    ...
    if (bp) {
        json = cJSON_Parse(bp);
        if (json) {
            String sp = JSONGetString(json, "apiBase");
            if (sp) {
                api = StrDuplicate(sp);
            }
            cJSON_Delete(json);
