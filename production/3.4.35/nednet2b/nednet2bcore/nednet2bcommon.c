...
void ApiInit(String url) {
    Logger(LOG_DEBUG, "ApiInit: Using URL '%s'", url);
...
int ParametersAdd(Parameters_t* pp, String key, String value) {
    ...
    if (!value) {
        dp[count].value = StrDuplicate("");
    } else {
        dp[count].value = StrDuplicate(value);
    }
...
int ParametersAddInt(Parameters_t* pp, String key, int value) {
    ...
    char buf[20];
    sprintf(buf, "%d", value);
    dp[count].value = StrDuplicate(buf);
    pp->numParameters = count + 1;
    if (pp->parameters) {
        free(pp->parameters);
    }
...
int ParametersFree(Parameters_t* pp) {
    if (pp && pp->parameters) {
        for (int i = 0; i < pp->numParameters; i++) {
            if (pp->parameters[i].value) {
                free(pp->parameters[i].value);
            }
        }
        free(pp->parameters);
        pp->parameters = NULL;
...
int MessagePost(Message_t* mp, int responseFlag) {
    ...
    Logger(LOG_DEBUG, "MessagePost()");
    buf = malloc(33000);
    if (!buf) {
        Logger(LOG_ERROR, "Could not allocate buffer");
        goto errorExit;
    }
    curl = curl_easy_init();
    if (!curl) {
...
int SimRead() {
    ...
    if (sTest) {
        ...
        // Turn of echo
        response = AtCommand("ATE0\r", "OK", NULL);
        free(response);
    ...
    if (rpp) {
        ...
        *response = 0;
        ...
        while (fgets(buf, sizeof(buf), p) != NULL) {
            if (strlen(buf) > (32000 - 1 - strlen(response))) {
                break;
            }
