    ...
    if (!sTest) {
        ...
        if (sp) {
            if (!strstr(sp, "WShighspeed")) {
                ...
                if (!strcmp(gModel, "gl-mifi")) {
                    String command = NULL;
                    command = StrConcat(command, "uci set wireless.default_radio0.ssid='WShighspeed-");
                    command = StrConcat(command, mac5);
                    command = StrConcat(command, "';");
                    command = StrConcat(command, "uci set wireless.guest2g.ssid='WSstandard-");
                    command = StrConcat(command, mac5);
                    command = StrConcat(command, "';");
                    ...
                    command = StrConcat(command, "uci set wireless.default_radio1.ssid='WShighspeed-");
                    command = StrConcat(command, mac5);
                    command = StrConcat(command, "';");
                    
                    command = StrConcat(command, "uci set wireless.guest5g.ssid='WSstandard-");
                    command = StrConcat(command, mac5);
                    command = StrConcat(command, "-5G';");
